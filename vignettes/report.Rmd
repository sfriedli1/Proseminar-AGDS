---
title: "Analysis of the Correlation of DTR and NDVI"
output: 
  html_document:
    code_folding: hide
...
# 1. Introduction
This analysis examines how the diurnal temperature range (DTR) defined as the difference between daily maximum and minimum temperature has evolved between 1981 and 2024 across 15 Meteo Swiss stations located in and around the Swiss Plateau. In a first step, DTR is calculated for each station and evaluated for long-term trends over the study period. Because changes in DTR can arise from shifts in either daily maximum temperature (Tmax), daily minimum temperature (Tmin), or both, the analysis additionally investigates trends in Tmax and Tmin to support the interpretation of observed DTR changes.
In a second step, the analysis explores whether differences in local vegetation conditions are related to differences in DTR behavior. Vegetation is quantified using the Normalized Difference Vegetation Index (NDVI) derived from MODIS satellite data. NDVI is extracted for the area surrounding each station, using a 250 m radius as an approximation of the immediate station environment. Building on these data, the relationship between NDVI and DTR is assessed using correlation and linear regression analysis.
For overview, the selected stations, including their abbreviations, full names, and geographic locations—are summarized below and visualized on an overview map.
...
**Look-Up Table Station Names and Abbreviations**
```{R}
source("../R/lookup_table.R")

knitr::kable(station_lookup,
             col.names = c("Abbreviation", "Station name"),
             caption = "Look-Up table fo station abbreviations and full names")
```
**Map of 15 Meteo Schweiz Stations**
```{R}
knitr::include_graphics("../figures/stations_map.png")
```
...
# 3. Analysis of NDVI and DTR
## 3.1 Analysis of Correlation between NDVI and DTR Trends
...
```
For further analysis of the correlation between the trends of the NDVI and the DTR a linear regression model was fitted. However, this shows there is no statistically significant linear relationship between the NDVI Sen's Slope and the trend of DTR per decade for all the seasons except for summer. For the summer the linear regression model is marginally statistically significant with a p-value of 0.098. The linear regression model shows that a higher NDVI Sen's Slope is associated with a higher DTR-Trend. It is important to note though that the adjusted R-squared of 0.13 indicates that the NDVI Sen's slope only explain 13% of the variation in the DTR-Trend. 

Concluding from the analysis of the DTR-Trend as well as the NDVI-Trend it seems plausible to say that there is no robust evidence for a correlation of the trends of these two variables. Likely both trends are probably rather explained by the more general large scale climatic changes. Especially the development of the NDVI is highly dependent on factors like precipitation and anthropogenic influence on land use.

## 3.2 Analysis Correlation DTR and NDVI
...
(ii) to what extent differences in DTR and its trends can be related to local vegetation conditions, represented by MODIS NDVI in a 250-m radius around each station (2001–2024).
...
In summary, the results show that **DTR in Switzerland has increased modestly over the last four decades**, mainly in late winter and spring, primarily due to a stronger warming of daytime than nighttime temperatures. Local vegetation characteristics, as captured by NDVI, appear to modulate DTR to some degree. "Greener" surroundings are generally associated with slightly smaller DTR but this influence is relatively weak compared to other factors and cannot be quantified with high statistical confidence for the current set of stations. Future work could extend the analysis to more stations, include additional surface and land-use variables (e.g. urbanisation, topography, soil moisture), and better account for the influence of anthropogenic activity on NDVI, in order to more clearly isolate and understand the role of vegetation in controlling the daily temperature range.
